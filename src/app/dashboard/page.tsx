...
export default function DashboardPage() {
  ...
  // جيب الـ sections من الـ API أو من الـ content مباشرة
  useEffect(() => {
    fetch("/api/content")
      .then((r) => r.json())
  ...
  const handleSpecialtyClick = (sectionName: string) => {
    ...
    if (slug) {
      router.push("/content/" + encodeURIComponent(slug));
    }
  };
                ...
                {searchResults.map((topic) => (
                  ...
                  >
                    <span style={{ fontSize: 16 }}>
                      {SECTION_ICONS[topic.section] || "📋"}
                    </span>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: "#0B1E3D" }}>
                        {topic.title}
                      </div>
