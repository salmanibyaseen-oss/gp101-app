  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (cancelled) return;
        setWatermarkText(info.watermark);
        setTitle(info.title);
        ...
        for (let i = 1; i <= total; i++) {
          ...
          canvas.style.cssText =
            "display:block;margin:0 auto 12px;max-width:100%;box-shadow:0 2px 16px rgba(0,0,0,0.3);border-radius:4px;";

          const ctx = canvas.getContext("2d")!;
          await page.render({ canvasContext: ctx, viewport }).promise;

          // watermark مرسوم على الـ canvas
          ctx.save();
          ...
          for (let row = -canvas.height; row < canvas.height; row += 110) {
            for (let col = -canvas.width; col < canvas.width; col += 300) {
              ctx.fillText(info.watermark, col, row);
            }
          }
          ctx.restore();
    ...
    >
      ...
      <div style={{
        ...
        {!loading && watermarkText && (
          <span style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", maxWidth: 180, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            🔒 {watermarkText}
          </span>
        )}
      </div>
